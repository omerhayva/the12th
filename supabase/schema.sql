...
create table if not exists public.players (
  ...
  display_name text not null default '12TH PLAYER',
  avatar_url text,
...
create table if not exists public.matches (
  ...
  status text not null default 'LIVE' check (status in ('LIVE','HT','FT')),
...
create table if not exists public.decisions (
  ...
  match_id text not null references public.matches(id) on delete cascade,
  window_id text not null references public.decision_windows(id) on delete cascade,
  minute integer not null,
...
create unique index if not exists decisions_player_window_unique
  on public.decisions(player_id, window_id);
...
select
  p.id,
  p.display_name,
  count(d.id)::integer as decisions,
  round(avg(d.points))::integer as iq,
  round((100.0 * avg(case when d.outcome = d.choice then 1 else 0 end)))::integer as accuracy,
  (count(d.id) < 5) as provisional
from public.players p
left join public.decisions d on d.player_id = p.id and d.points is not null
...
-- Public match data can be read by the client.
