...
export const bsdFootballProvider: FootballProvider = {
  ...
  async getEvents(matchId) { const payload = await bsdFetch(`/events/${encodeURIComponent(matchId)}/incidents/`); return results(payload).map(mapIncident).sort((a,b) => a.minute-b.minute); },
  async getDecisionWindows(matchId) { const match = await this.getMatch(matchId); return !match || match.status === 'FT' ? [] : predictionWindows(match); },
};
