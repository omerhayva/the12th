...
export async function GET(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (!process.env.BSD_FOOTBALL_API_KEY) return NextResponse.json({ error: 'LIVE_FEED_NOT_CONFIGURED' }, { status: 503 });
  try {
    const match = await bsdFootballProvider.getMatch(id);
    if (!match) return NextResponse.json({ error: 'MATCH_NOT_FOUND' }, { status: 404 });
    const [events, windows] = await Promise.all([bsdFootballProvider.getEvents(id), bsdFootballProvider.getDecisionWindows(id)]);
    return NextResponse.json({ source: 'bsd-live', match, events, windows, decisions: [] });
  } catch (error) {
    console.error(`BSD match ${id} failed:`, error);
    return NextResponse.json({ error: 'LIVE_FEED_UNAVAILABLE' }, { status: 503 });
  }
}
