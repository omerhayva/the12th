...
export default function MatchPage({ params }: { params: { id: string } }) {
  ...
  const submitDecision = () => {
    ...
    const nextDecision = { windowId: activeWindow.id, matchId: match.id, minute: activeWindow.minute, choice: selected, lockedAt: Date.now() };
  ...
  return (
    <main className="page"><div className="shell">
      ...
      <section className="hero matchHero"><div className="matchTeams"><div className="teamHero"><img src={match.homeLogo} alt={match.home} /><span>{match.home}</span></div><div className="scoreHero"><div className="eyebrow">LIVE MATCH / {match.minute}&apos;</div><strong>{match.homeScore} — {match.awayScore}</strong><span>12TH LIVE</span></div><div className="teamHero"><img src={match.awayLogo} alt={match.away} /><span>{match.away}</span></div></div><p className="sub">Maçın kritik anında sıra sende. Kararını ver, sonra gerçek oyunla karşılaştıralım.</p></section>
