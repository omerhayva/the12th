...
const eventButtons: { type: MatchEventType; title: string }[] = [
  ...
  { type: 'CARD', title: 'KART' },
  { type: 'SUBSTITUTION', title: 'DEĞİŞİKLİK' },
  { type: 'POSSESSION', title: 'TOP KAZANMA' },
  { type: 'END', title: 'MAÇI BİTİR' },
];
...
export default function AdminPage() {
  ...
  function addEvent(type: MatchEventType, title: string) {
    ...
    const event: MatchEvent = {
      ...
      minute,
      type,
      team: type === 'END' ? undefined : team,
      title: type === 'GOAL' ? `${team === 'HOME' ? match.home : match.away} gol attı` : `${team === 'HOME' ? match.home : match.away} · ${title}`,
    };
    ...
    let homeScore = state.meta.homeScore;
    let awayScore = state.meta.awayScore;
    let status = state.meta.status;
    ...
    const nextState: LiveMatchState = {
      meta: { minute, homeScore, awayScore, status },
  ...
  return <main className="page-shell">
    ...
    <section className="hero-block">
      <div><div className="eyebrow">MATCH CONTROL</div><h1>Canlı Maç Yönetimi</h1><p className="muted">Gerçek olayları gir. Taraftar kararını olayla eşleştir ve Football IQ puanını üret.</p></div>
      <div className="admin-controls">
        ...
        <button className="secondary-btn" onClick={handleReset}>DEMOYU SIFIRLA</button>
      </div>
    </section>
    ...
    <section className="admin-grid">
      <div className="panel">
        ...
        <div className="timeline">{currentEvents.slice().reverse().map((event) => <div className="timeline-row" key={event.id}><strong>{event.minute}&apos;</strong><span>{event.title}</span><small>{event.team ? `${event.team} · ` : ''}{event.type}</small></div>)}</div>
      </div>
      <div className="panel">
        ...
        {windows.map((item) => { const linked = state.decisions.find((d) => d.windowId === item.id); const result = linked && item.resolved ? evaluateDecision(linked.choice, item) : null; return <div className="decision-window" key={item.id}>
          ...
          {linked && !item.resolved && <button className="primary-btn" onClick={() => resolveWindow(item.id)}>SONUCU İŞLE →</button>}
