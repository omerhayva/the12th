...
export default function AdminPage() {
  ...
  useEffect(() => {
    setAdminKey(window.localStorage.getItem('the12th:admin-key') ?? '');
  }, []);

  useEffect(() => {
    const next = readLiveState(matchId); setState(next); setMinuteInput(String(next.meta.minute));
  }, [matchId]);
  ...
  async function addEvent(type: MatchEventType, title: string) {
    ...
    const target = windows.find((item) => !item.resolved && item.minute <= minute);
    let nextWindows = windows; let nextDecisions = state.decisions;
    let homeScore = state.meta.homeScore; let awayScore = state.meta.awayScore; let status = state.meta.status;
    if (type === 'GOAL') team === 'HOME' ? homeScore += 1 : awayScore += 1;
    ...
    if (target && suggestedChoice) {
      ...
      if (linked) {
        linkedId = linked.id; resolution = resolveDecision(linked, event, target.correctChoice);
        nextDecisions = state.decisions.map((decision) => decision.windowId === target.id ? { ...decision, points: resolution!.points, outcome: resolution!.outcome, label: resolution!.label, eventMinute: event.minute, eventType: event.type } : decision);
    ...
    const persisted = await persistAdmin({ action: 'event', minute, type, team: event.team, title: event.title, description: event.description });
  ...
  async function resolveWindow(windowId: string) {
    ...
    const nextState = { ...state, windows: nextWindows, decisions: nextDecisions }; setState(nextState); writeLiveState(matchId, nextState);
  ...
  return <main className="page-shell">
    ...
    <section className="hero-block"><div><div className="eyebrow">MATCH CONTROL</div><h1>Canlı Maç Yönetimi</h1><p className="muted">Gerçek olayları gir. Taraftar kararını olayla eşleştir ve Football IQ puanını üret.</p></div>
      <div className="admin-controls"><input aria-label="Admin anahtarı" className="select" type="password" placeholder="ADMIN KEY" value={adminKey} onChange={(e) => saveAdminKey(e.target.value)} />
        <select value={matchId} onChange={(e) => setMatchId(e.target.value)} className="select">{matches.map((item) => <option key={item.id} value={item.id}>{item.home} — {item.away}</option>)}</select>
        <select value={team} onChange={(e) => setTeam(e.target.value as 'HOME' | 'AWAY')} className="select"><option value="HOME">EV SAHİBİ · {match.home}</option><option value="AWAY">DEPLASMAN · {match.away}</option></select>
        <input aria-label="Maç dakikası" className="select" type="number" min="0" max="120" value={minuteInput} onChange={(e) => setMinuteInput(e.target.value)} />
        <button className="secondary-btn" onClick={handleReset}>DEMOYU SIFIRLA</button></div>
    </section>
    ...
    <div className="admin-note">{apiStatus} • Production admin API anahtar ile korunur.</div>
  </main>;
}
